...
class User extends CI_Controller {
	...
	public function register(){
		...
		if($result){
			...
			if ($this->email->send()) {
       		 redirect(base_url().'welcome/index/2/');
    } else {
        ...
        /*show_error($this->email->print_debugger());*/
			...
			$data = array();
               ...
               redirect(base_url().'welcome/index/1/#register');
	...
	public function forgetPasswordLink(){
		...
		if($result){
			$data = array('message' => 'Reset Password link has been sent to your email','type' => 'success');
		}else{
			$data = array('message' => 'Error in sending email.Please contact our support team','type' => 'warning');
		}
	...
	public function changePasswordPost(){
			...
			if($result){
				...
				$data = array('message' => 'Error in changing password.Please contact support','type' => 'warning');
			}
			$this->load->view('user/login',$data);			
	}
